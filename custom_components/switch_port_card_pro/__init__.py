...
async def async_install_frontend_resource(hass: HomeAssistant):
    ...
    def install():
        ...
        target_dir = hass.config.path(
            "www", "community", "switch_port_card_pro_card"
        )  # legacy naming
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    await async_install_frontend_resource(
        hass
    )  # copy to card to the location it is supposed to be at.
    ...
    if is_first_install:
        ...
        new_options.pop(
            "re_detect_ports", None
        )  # clear flag — listener not yet registered so no reload loop
        ...
        if detected:
            ...
            if manufacturer != "Unknown" and old_manufacturer != manufacturer:
                new_options["manufacturer"] = manufacturer
                _LOGGER.info(
                    "Updated manufacturer for %s: %s → %s",
                    host,
                    old_manufacturer,
                    manufacturer,
                )
    ...
    base_oids = {
        ...
        "poe_power": entry.options.get(
            "oid_poe_power", DEFAULT_BASE_OIDS.get("poe_power", "")
        ),
        "poe_status": entry.options.get(
            "oid_poe_status", DEFAULT_BASE_OIDS.get("poe_status", "")
        ),
        "poe_class": entry.options.get(
            "oid_poe_class", DEFAULT_BASE_OIDS.get("poe_class", "")
        ),
        "port_custom": entry.options.get(
            "oid_port_custom", DEFAULT_BASE_OIDS.get("port_custom", "")
        ),
    }

    system_oids = {
        "cpu": entry.options.get("oid_cpu", DEFAULT_SYSTEM_OIDS.get("cpu", "")),
        "memory": entry.options.get(
            "oid_memory", DEFAULT_SYSTEM_OIDS.get("memory", "")
        ),
        "memory_total": entry.options.get(
            "oid_memory_total", DEFAULT_SYSTEM_OIDS.get("memory_total", "")
        ),
        "firmware": entry.options.get(
            "oid_firmware", DEFAULT_SYSTEM_OIDS.get("firmware", "")
        ),
        "hostname": entry.options.get(
            "oid_hostname", DEFAULT_SYSTEM_OIDS.get("hostname", "")
        ),
        "uptime": entry.options.get(
            "oid_uptime", DEFAULT_SYSTEM_OIDS.get("uptime", "")
        ),
        "custom": entry.options.get(
